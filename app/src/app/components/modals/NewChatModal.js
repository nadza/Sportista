...
const content = {
    ...
    bs: {
        title: 'Napravi novi razgovor',
        name: 'Naziv',
        user: 'Korisnik ili korisnici'
    }
  };
...
export default function NewChatModal() {
    ...
    async function fetchData() {
        try {
            const users = await axios.get("http://localhost:8000/korisnici", { withCredentials: true });
    ...
    const handleSubmit = async () => {
        ...
        if (loginCookie) {
           username = JSON.parse(decodeCookieValue(loginCookie)).username;
        }
        ...
        try {
            ...
            await axios.post(`http://localhost:8000/create-chat/${username}`, usersWithName, { withCredentials: true });
            handleClose();
        } catch (error) {
