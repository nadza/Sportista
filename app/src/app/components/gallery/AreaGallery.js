...
export default function AreaGallery({ id }) {
  ...
  const fetchData = React.useCallback(async () => {
    setLoading(true);
    try {
        console.log("TUUUUUUUUU BRATE", id);
      const response = await axios.get(`http://localhost:8000/get_area_photos/${id}`, { withCredentials: true });
  ...
  async function fetchOldDeleteNow() {
    ...
    try {
      ...
      const response = await axios.delete(`http://localhost:8000/delete_area_pictures/${id}`, {
        data,
  ...
  const handleFileUpload = async (e) => {
    ...
    try {
      ...
      formData.append('folder', 'area_pictures');
      ...
      const response = await axios.post('http://localhost:8000/upload_area_pictures', formData, {
        headers: {
