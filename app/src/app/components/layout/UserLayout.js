...
function MenuIconSideNavExample({ orientation, setContent }) {
  ...
  useEffect(() => { 
    ...
    if (loginCookie) {
      ...
      const user_role = JSON.parse(decodeCookieValue(loginCookie)).user_role;
  ...
  const handleMenuClick = (index) => {
    ...
    switch (index) {
      case 0:
        setContent(<UserProfileCard />);
        break;
      case 1:
        if (userRole ==='Player') {
          setContent(<CardsLayout type="appointment" show="false"/>);
        } else if (userRole === 'Company') {
          setContent(<CardsLayout type="area" />);
        }
        break;
      case 2:
        if (userRole ==='Player') {
          setContent(<CardsLayout type="review" />);
        } else if (userRole === 'Company') {
          setContent(<DisplayCompanyCard name={userName}/>)
        }
        break;
      case 3:
        if (userRole === 'Company') {
          setContent(<Gallery name={userName}/>)
        }
        break;
      default:
        setContent(null);
...
const UserLayout = () => {
  const [content, setContent] = useState(<UserProfileCard />);
